...
| --- | --- | --- |
| **CORS block in browser** (e.g. preflight fails) | The domain hosting your frontend application is not registered in the API key's allowed origins list. | 1. Go to the CMS dashboard.<br>2. Open **Settings → API Keys** and edit the key you are using.<br>3. Under **Allowed Origins**, add your application's domain (such as `https://example.com` or `http://localhost:3000`).<br>4. Save the key. The update propagates to the edge in seconds. |
...
| **400 Bad Request** during filter or sort query | You are attempting to filter (`?filter[field][eq]`) or sort (`?sort=field`) using a field that is not marked as filterable. | 1. Go to the **Schema Designer**.<br>2. Select the collection and edit the target field.<br>3. Toggle the **Filterable** switch on.<br>4. Select **Publish Schema** to generate the database column and SQLite index. |
...
| **API Key returns 403 Forbidden** | The key does not have the scope required for the endpoint (e.g. using a `public-read` key to save a draft). | 1. Generate a new API key with the `write` or `admin` scope.<br>2. Check that the key's allowed collections configuration includes the collection you are querying. |
