...
| **400 Bad Request** during filter or sort query | You are attempting to filter (`?filter[field]=value`) or sort (`?sort=field`) using a field that is not marked as filterable, or you are using unsupported V2 operators like `[eq]` or `[gt]`. | 1. Go to the **Collections** workspace tab.<br>2. Select the collection and toggle the view to **Schema Designer**.<br>3. Edit the target field and toggle the **Filterable** switch on.<br>4. Click **Publish Schema** at the bottom of the left collections sidebar to generate the database column and index.<br>5. Ensure your API request matches the exact-match format: `?filter[field_slug]=value`. |
...
| **413 Payload Too Large** (during upload) | The uploaded file exceeds the platform's **25 MB** limit. | 1. Compress archives or downscale media assets before uploading.<br>2. Ensure uploaded files are under the 25 MB size limit.<br>3. If uploading videos or large binaries, host them on external storage and save the URLs in a text field. |
| **413 Payload Too Large** (image retrieval) | The source image requested for on-demand resizing exceeds the **8 megapixel (8 MP)** edge CPU protection cap. | 1. Downscale the source image before uploading so its total pixels are under 8,000,000.<br>2. Request the file *without* resizing query parameters to retrieve the original image directly from storage without edge transformation compute. |
